...
exports.compile = function (options, complete) {
	...
	async.waterfall([
		...
		function writeBundle (source, next) {
            ...
            fs.mkdir(path.join(nodeCompiler.dir, "lib"));
			fs.writeFile(bundlePath, source, {flag:'w'}, next);
            _log("This worked ok");
		},
	...
	async.waterfall([
		...
		function downloadNode (next) {
			...
			output.on("close", function () { 
                _log("Finished downloading Node");
                next(); 
            });
		},
...
function _monkeyPatchMainJs (compiler, complete) {
    ...
    fs.mkdir(path.join(compiler.dir, "src"));
